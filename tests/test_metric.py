...
def mock_processor():
    ...
    processor.batch_decode = MagicMock(
        side_effect=lambda ids, **kwargs: [
            f"decoded_string_{i}" for i in range(len(ids))
        ]
    )
    return processor
...
def mock_eval_prediction():
    ...
    pred.predictions = np.array(
        [[[0.1, 0.8, 0.1], [0.9, 0.05, 0.05]], [[0.2, 0.2, 0.6], [0.1, 0.7, 0.2]]]
    )
    # Mock label IDs, including the loss masking token -100
    pred.label_ids = np.array([[1, -100], [2, 1]])
    return pred
...
def test_compute_metrics_success(
    ...
    expected_labels = np.array([[1, 0], [2, 1]])
